...
stack을 사용하면 쉽게 풀이할수 있다.

moves가 열이므로 board의 열에 접근하여 값이 있을때까지 행을 순회한다.

만약 0이 아닌 값이 있다면 stackArr(새로운 배열)에 추가하며 board의 해당 원소는 0으로 바꿔준다.

그리고 만약 stackArr에 원소를 추가할때 stackArr의 마지막이 원소가 추가 할 원소의 값과 같다면 

stackArr의 마지막 원소를 지우고 count에 +2를 해준다
